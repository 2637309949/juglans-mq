...
Queue.prototype.loop = function () {
  ...
  for (const item of this.tactics) {
    const inv = setInterval((item => async function ({type, tactic: { ctCount }} = item) {
      let exector
      if (!type) {
      ...
      for (const exec of exector) {
        ...
        const proccessing = await _this.model.findTask({ type, status: Queue.status.PROCESSING })
        let [task] = await _this.model.findTask({ type, status: Queue.status.INIT })
